...
def main(config: dict, logger) -> None:
    if config["data_path"]:
        avazu_path = config["data_path"]
    else:
        avazu_path = os.path.join("data", "avazu", "train.csv")

    logger.info(f"Loading the Avazu data from {avazu_path}")

    avazu_dataset = AvazuCTRDataset(
        data_path=avazu_path, sample_size=config["sample_size"], seed=config["seed"]
    )

    logger.info(f"Preprocessed the Avazu data.")
    ...
    avazu_train, avazu_test = train_test_split(
        avazu_dataset.data, test_size=config["test_size"], random_state=config["seed"]
    )
    train_model_input = {name: avazu_train[name] for name in feature_names}
    test_model_input = {name: avazu_test[name] for name in feature_names}
...
if __name__ == "__main__":
    ...
    logfname = f"avazu_deepfm_{now.strftime('%Y%m%d_%H%M%S')}.log"
    logger = setup_logger(os.path.join("src", "logs", logfname))

    parser = argparse.ArgumentParser(description="Avazu - DeepCTR DeepFM Training")
